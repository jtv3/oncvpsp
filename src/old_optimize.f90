...
! in orthonormal and spherical Bessel function bases
 ...
 subroutine old_optimize(nnull,nbas,pswf0_sb,pswf0_or,nqout,qout,&
...
!      SUBROUTINE DSYEV( JOBZ, UPLO, N, A, LDA, W, WORK, LWORK, INFO )
 ...
 if(info .ne. 0) then
  write(6,'(a,i4)') 'old_optimize: redisual-energy eigenvalue ERROR, info=',info
  stop
 end if
...
!appropriate to make it "dependent."
 ...
 if(nnull==2) then
  do kk=-nopt,nopt
   pswfopt(2)=dpso*kk
   tt=yy**2-pswfopt(2)**2
   if(tt<0.d0) tt=0.d0  ! account for small negative round-off
   pswfopt(1)=sqrt(tt)
   eropt(1)=eresid0(1)
   do ii=1,nnull
...
&                    +eresideval(ii)*pswfopt(ii)**2)
  ...
  write(6,'(/a,a)') 'old_optimize: ERROR only set up  for E_resid optimization on', &
&  ' <=5  variables'
...
&             -0.5d0*rtsgn*eresiddot_ev(1)/pswfopt(1))
   tt=tt-pswfopt(ii)**2 
   if(tt<0.0d0) then 
    write(6,'(/a)') 'old_optimize: ERROR norm constraint violated (line 260)'
    stop
   end if
